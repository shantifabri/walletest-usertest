...
RSpec.describe CreditAccount, type: :model do
  ...
  describe 'columns' do
    ...
    it do
      expect(credit_account).to have_db_column(:currency)
        .of_type(:string)
    end

    it do
      expect(credit_account).to have_db_column(:liquid_balance)
        .of_type(:integer)
    end

    it do
      expect(credit_account).to have_db_column(:illiquid_balance)
        .of_type(:integer)
    end

    it do
      expect(credit_account).to have_db_column(:credit_limit)
        .of_type(:integer)
    end
